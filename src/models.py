...
class ArrowSetup(SQLModel, table=True):
    ...
    shafts: list["ArrowShaft"] = Relationship(
        back_populates="arrow_setup", sa_relationship_kwargs={"cascade": "all, delete"}
    )
...
class Session(SQLModel, table=True):
    ...
    bow_id: str | None = Field(default=None, foreign_key="bowsetup.id")
    arrow_id: str | None = Field(default=None, foreign_key="arrowsetup.id")
    ...
    # Relationships
    bow: BowSetup | None = Relationship(back_populates="sessions")
    arrow: ArrowSetup | None = Relationship(back_populates="sessions")
...
class Shot(SQLModel, table=True):
    ...
    arrow_number: int | None = Field(default=None, description="The number marked on the arrow shaft")
    shot_sequence: int | None = Field(default=None, description="Deterministic shot order within end (0-indexed)")
