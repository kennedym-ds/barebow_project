...
def compute_rayleigh_sigma_with_ci(
    xs: np.ndarray, ys: np.ndarray, confidence: float = 0.95
) -> dict:
    """
...
def compute_accuracy_precision_ratio(
...
def compute_ewma(values: list[float], lam: float = 0.2) -> dict:
    ...
    if len(values) < 2:
        return {
            "ewma": values, "ucl": values, "lcl": values,
            "mean": values[0] if values else 0.0, "sigma": 0.0
        }
...
def compute_within_end_trend(shots_by_position: dict[int, list[float]]) -> dict:
    ...
    if diff < 0.2:
        ...
        interp = f"Last arrow is weakest ({worst['avg_score']:.2f} avg). May indicate rushing or fatigue within the end."
    else:
...
def compute_confidence_ellipse(xs: np.ndarray, ys: np.ndarray, coverage: float = 0.9) -> dict:
    ...
    if len(xs) < 3:
        return {
            ...
            "semi_major": 0.0, "semi_minor": 0.0, "angle_deg": 0.0,
            "coverage": coverage, "correlation": 0.0,
        }
...
def detect_fliers(xs: np.ndarray, ys: np.ndarray, threshold_sigma: float = 2.5) -> dict:
    ...
    if n < 5:
        ...
        return {
            ...
            "clean_sigma": round(full_drms, 3), "full_sigma": round(full_drms, 3),
    ...
    # Also use simple sigma-based threshold as fallback
    radii = np.sqrt((xs - mean[0])**2 + (ys - mean[1])**2)
    sigma_r = float(np.std(radii))
...
def compute_multi_distance_profile(
    ...
    if len(distance_data) < 2:
        return {
            "distances": distance_data, "mean_sigma_theta": 0.0,
            "distance_effect": False, "interpretation": "Need data at ≥2 distances"
        }
...
def compute_equipment_comparison(
    setup_a_scores: list[float], setup_a_sigmas: list[float], setup_a_name: str,
    setup_b_scores: list[float], setup_b_sigmas: list[float], setup_b_name: str,
) -> dict:
    """
    ...
    if len(setup_a_scores) < 2 or len(setup_b_scores) < 2:
        return {
            "setup_a": setup_a_name, "setup_b": setup_b_name,
            "score_diff": 0.0, "score_p_value": 1.0, "score_cohens_d": 0.0,
            "sigma_diff": 0.0, "sigma_p_value": 1.0,
    ...
    _, p_sigma = scipy_stats.ttest_ind(a_sig, b_sig, equal_var=False) if len(setup_a_sigmas) >= 2 and len(setup_b_sigmas) >= 2 else (0, 1.0)
    ...
    if score_sig and score_diff > 0:
        ...
        interp = f"{setup_b_name} scores significantly higher than {setup_a_name} (p={p_score:.3f}, d={abs(cohens_d):.2f})"
    else:
